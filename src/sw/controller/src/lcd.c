...
void lcd_write_byte(LCD *lcd, u8 data) {
  ...
  for (int i = 0; (!tx_done || XIic_IsIicBusy(lcd->iic)) && i < MAX_WAIT_ITER;
       i++) {
  }
  lcd_end_tx(lcd);
}
...
void lcd_clear(LCD *lcd) {
  lcd_command(lcd,
              LCD_CLEARDISPLAY); // clear display, set cursor position to zero
...
static int init_interrupts(XIic *iic) {
  ...
  Status = XIntc_Connect(&intc, IIC_INTR_ID,
                         (XInterruptHandler)XIic_InterruptHandler, iic);
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
  ...
  XIic_SetSendHandler(iic, iic, (XIic_Handler)tx_handler);
  XIic_SetRecvHandler(iic, iic, (XIic_Handler)rx_handler);
  XIic_SetStatusHandler(iic, iic, (XIic_StatusHandler)status_handler);
